...
vi.mock("../load-script", () => ({
  loadCollectorsScript: vi.fn().mockResolvedValue({
    getToken: vi.fn().mockReturnValue("01ARYZ6S41TSV4RRFFQ69G5FAV"),
  }),
...
describe("WorkOSRadar", () => {
  ...
  it("tokenReady is true after successful init", async () => {
    const radar = WorkOSRadar.init({ clientId: "client_test_123" });
    await radar.getToken();
    expect(radar.tokenReady).toBe(true);
  });

  it("tokenReady is false before init completes", () => {
    const radar = WorkOSRadar.init({ clientId: "client_test_123" });
    expect(radar.tokenReady).toBe(false);
  });
  ...
  it("tokenReady is false when CDN script fails to load", async () => {
    vi.mocked(loadCollectorsScript).mockRejectedValueOnce(new Error("network"));
